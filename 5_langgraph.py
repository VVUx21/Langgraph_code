...
def _extract_feedback_score(out: Any) -> tuple[str, int]:
    if isinstance(out, dict):
        feedback = str(out.get("feedback", ""))
        score = int(out.get("score", 0))
        return feedback, score

    feedback = str(getattr(out, "feedback", ""))
    score = int(getattr(out, "score", 0))
    return feedback, score
...
def evaluate_language(state: UPSCState):
    ...
    feedback, score = _extract_feedback_score(out)
